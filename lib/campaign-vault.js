...
export function clipContentFingerprint(clip = {}) {
  return stableHash({
    transcript: String(clip.transcript || ""),
    timestamps: clip.sourceTimestamps || {},
    captions: clip.captionPackage || {},
    hook: String(clip.hook || ""),
    title: String(clip.title || ""),
    source: {
      projectId: clip.sourceProjectId || null,
      mediaId: clip.sourceMediaId || null,
      clipId: clip.sourceClipId || null,
    },
    processingVersion: String(clip.processingVersion || "autoklip-v1"),
  });
}
...
export function vaultClipForClient(clip, { includePrivate = false } = {}) {
  ...
  const view = {
    ...
    contentFingerprint: clip.contentFingerprint || null,
...
export async function importProjectCandidates(store, {
  ...
  for (const clip of selected) {
    ...
    const contentFingerprint = clipContentFingerprint({
      ...payload,
      sourceProjectId: payload.sourceProjectId,
      sourceMediaId: payload.sourceMediaId,
      sourceClipId: payload.sourceClipId,
    });
    const previousFingerprint = existing?.contentFingerprint || (existing ? clipContentFingerprint(existing) : null);
    const fingerprintChanged = Boolean(previousFingerprint && previousFingerprint !== contentFingerprint);
    const mustReapprove = Boolean(existing && existing.approvalStatus === "APPROVED" && fingerprintChanged);
    const saved = await store.saveClip({
      id: existing?.id || store.createId(),
      campaignId: campaign.id,
      workspaceId: campaign.workspaceId,
      approvalStatus: mustReapprove || !existing ? "CANDIDATE" : existing.approvalStatus,
      usageCount: existing?.usageCount || 0,
      approvedBy: mustReapprove ? null : existing?.approvedBy || null,
      approvedAt: mustReapprove ? null : existing?.approvedAt || null,
...
export async function reviewVaultClip(store, { clip, decision, actorId }) {
  ...
  const saved = await store.saveClip({
    ...clip,
    approvalStatus,
    contentFingerprint: clip.contentFingerprint || clipContentFingerprint(clip),
...
export async function incrementClipUsage(store, clip) {
  if (store.incrementClipUsage) {
    return store.incrementClipUsage(clip.id, clip.workspaceId);
  }
