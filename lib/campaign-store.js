...
import { LOCAL_WORKSPACE_ID, nowIso } from "./campaign-constants.js";
...
export function createMemoryCampaignStore({ persistDir = null } = {}) {
  ...
  const store = {
    ...
    async getSubmissionByCanonicalUrl(canonicalUrl) {
      return clone(submissions.findOne((item) => item.canonicalUrl === canonicalUrl));
    },
    async saveSubmission(submission) {
      const saved = submissions.put({ ...submission, updatedAt: nowIso() });
      persist();
      return clone(saved);
    },
    ...
    async saveFeatures(record) {
      const saved = features.put({ ...record, updatedAt: nowIso() });
      persist();
      return clone(saved);
    },
    ...
    async saveObservation(observation) {
      const saved = observations.put(observation);
      persist();
      return clone(saved);
    },
