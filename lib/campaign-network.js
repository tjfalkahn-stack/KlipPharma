...
export function normalizeProfileInput(body = {}, { existing = null, userId } = {}) {
  ...
  return {
    ...
    username,
    socialHandles,
    connectedPlatforms: uniqueStrings(body.connectedPlatforms ?? existing?.connectedPlatforms ?? []),
...
export async function upsertKlipperProfile(store, { userId, workspaceId, input }) {
...
export async function joinCampaign(store, {
  ...
  if ((campaign.allowedRegions || []).length && region && !campaign.allowedRegions.includes(String(region).toUpperCase())) {
    throw new CampaignError("This campaign is not open in that region.", 403);
  }
