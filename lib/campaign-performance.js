...
export async function commandCenter(store, { workspaceId, campaigns }) {
  const live = campaigns.filter((item) => ["LIVE", "PAUSED"].includes(item.status));
  const reports = [];
  for (const campaign of live) {
    reports.push({ campaign, analytics: await campaignAnalytics(store, { campaign, includeFinancials: true }) });
  }
