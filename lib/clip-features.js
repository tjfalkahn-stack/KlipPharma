...
export function extractClipFeatures({ clip, job = null, campaign = null, extras = /** @type {Record<string, any>} */ ({}) }) {
...
export async function recordPerformanceObservation(store, {
  campaign, clip, submission, metrics, verificationVersion = 1,
}) {
  if (submission?.id && store.getObservationForSubmission) {
    const existingObservation = await store.getObservationForSubmission(submission.id, verificationVersion);
