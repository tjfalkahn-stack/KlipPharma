...
import pg from "pg";
import { campaignNetworkSchemaSql } from "./campaign-schema.js";
...
export async function initializeDatabase() {
  ...
  `);
  await initializeUploadPersistenceSchema();
  await initializeCampaignNetworkSchema();
...
export async function initializeCampaignNetworkSchema(targetPool = pool) {
  if (!targetPool) return;
  await targetPool.query(campaignNetworkSchemaSql());
}

export async function initializeUploadPersistenceSchema(targetPool = pool) {
  if (!targetPool) return;
...
export async function getSocialConnection(userId, provider, { workspaceId = null } = {}) {
  ...
  const connection = socialConnection(rowToSocialConnection(result.rows[0]));
  if (workspaceId && connection.workspaceId && connection.workspaceId !== workspaceId) {
    return connection;
  }
  return connection;
}
...
export async function listSocialConnections(userId) {
  if (!pool || !userId || userId === "local-owner") return [];
  const result = await pool.query(
    `SELECT user_id, provider, workspace_id, provider_user_id, profile, scopes,
       access_token, refresh_token, access_token_expires_at, refresh_token_expires_at,
       connected_at, updated_at
     FROM social_connections WHERE user_id = $1 ORDER BY provider`,
    [userId],
  );
  return result.rows.map((row) => socialConnection(rowToSocialConnection(row)));
}
